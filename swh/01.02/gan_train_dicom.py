...
from datasets.dataset import DICOM_Dataset, collate_fn_dicom, unmap_data
from model.unet_cbam import Generator, Discriminator
...
def get_args():
    ...
    parser.add_argument('--model_path', type=str, default=r'./output/model_1.pth', help='model path')
...
def train(args, generator, registor, discriminator, train_dataloader, val_dataloader, device=None, logger=None):
    # Define optimizers
    optimizer_G = optim.Adam([{'params':generator.parameters(), 'lr':args.lr_g},
                            {'params':registor.parameters(), 'lr':args.lr_g}], betas=(0.9,0.999))
    optimizer_D = optim.Adam(discriminator.parameters(), lr=args.lr_d, betas=(0.5,0.9))
    frequency = 5
    lr_scheduler_g = scheduler(optimizer_G, warm_up_steps=330//frequency, total_steps=2300, base_lr=args.lr_g, min_lr=1e-6)
    lr_scheduler_d = scheduler(optimizer_D, warm_up_steps=330//frequency, total_steps=2300, base_lr=args.lr_d, min_lr=1e-6)

    # resume
    if args.resume:
        print('loading pretrained weights')
        load_checkpoint(args.model_path, generator=generator, discriminator=discriminator, registor=registor)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (ct_data, cta_data) in loop:
            ...
            # mseloss = []
            psnr_list = []
            ct_img = ct_data['img'].to(device).permute(3,0,1,2)
            cta_img = cta_data['img'].to(device).permute(3,0,1,2)
            ...
            if (i+1) % frequency != 0:
                ...
                for ct, cta in zip(split_ct_img, split_cta_img):    
                    ...
                    mse_loss = F.mse_loss(reg_fake_cta, cta)
    ...
    for i, (ct_data, cta_data) in loop:
            ...
            for ct, cta in zip(split_ct_img, split_cta_img):
                ...
                with torch.no_grad():
                    ...
                    fake_cta = generator(ct) 
                    reg_fake_cta = transformer(fake_cta, flow)
                gen_cta = torch.cat((gen_cta, reg_fake_cta), dim=0)
            ...
            gen_cta = unmap_data(gen_cta, args.wc, args.ww)
...
if __name__ == '__main__':
    ...
    train_dataset = DICOM_Dataset(ct_dir=args.train_ct_dir, cta_dir=args.train_cta_dir, args=args)
    val_dataset = DICOM_Dataset(ct_dir=args.test_ct_dir, cta_dir=args.test_cta_dir, args=args)
    ...
    train_dataloader = DataLoader(dataset=train_dataset, batch_sampler=batch_sampler_train, num_workers=2, 
                                  pin_memory=True, collate_fn=collate_fn_dicom)
    val_dataloader = DataLoader(dataset=val_dataset, batch_size=args.batch_size, sampler=sampler_val,
                                num_workers=2, collate_fn=collate_fn_dicom, drop_last=False) 
    ...
    if args.val:
        ...
        if args.resume:
            print('loading pretrained weights')
            load_checkpoint(args.model_path, generator=generator, registor=registor)
        val(args, generator, registor, val_dataloader, device)
    else:
        print("train mode")
