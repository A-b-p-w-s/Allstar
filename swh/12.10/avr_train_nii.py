...
# from unet_cbam import UNet_CBAM, Discriminator2
# from transformer_2d import Transformer_2D
from model.avr import AVR
from model.resnet50 import ResNet50Backbone, ResNet50layers
from criterion import (ssim_metric, psnr, mse)
...
def get_args():
    ...
    parser.add_argument('--lr_drop', type=int, default=15, help='lr drop step')
    ...
    parser.add_argument('--train_ct_dir', type=str, default=r'E:\data\CTA\train\A3', help='A3 is CT dir')
    parser.add_argument('--train_cta_dir', type=str, default=r'E:\data\CTA\train\A0', help='A0,A1,A2 is CTA dir')
    parser.add_argument('--val_ct_dir', type=str, default=r'E:\data\CTA\val\A3', help='A3 is CT dir')
    parser.add_argument('--val_cta_dir', type=str, default=r'E:\data\CTA\val\A0', help='A0,A1,A2 is CTA dir')
    parser.add_argument('--test_ct_dir', type=str, default=r'E:\data\CTA\test\A3', help='A3 is CT dir')
    parser.add_argument('--test_cta_dir', type=str, default=r'E:\data\CTA\test\A0', help='A0,A1,A2 is CTA dir')
    
    # trian params
    parser.add_argument('--resume', action='store_true', help='resume')
    parser.add_argument('--val', action='store_true', help='val')
    parser.add_argument('--model_path', type=str, default=r'./output/model_26.pth', help='model path')
...
def define_optimizers(generator, registor, discriminator, learning_rate):
    ...
    # optimizer_C = optim.SGD(registor.parameters(), lr=learning_rate, weight_decay=0.0001)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, (ct_data, cta_data) in loop:
            ...
            for ct, cta in zip(split_ct_img,split_cta_img):
                ct, cta = ct.repeat(1,3,1,1).to(device), cta.to(device) 
                optimizer_G.zero_grad()
                with torch.no_grad():
                    laten_x = backbone(ct).detach()

                fake_cta = generator(laten_x)
                # flow = registor(fake_cta, cta_imgs)
                # reg_fake_cta = transformer(fake_cta, flow)
                mae_loss = mse(fake_cta, cta)
                Gen_loss.append(mae_loss.item())
            ...
            if i % 10 == 0:
                logger.info(f'G_Loss: {np.mean(Gen_loss):.4f}')
            
       
        print(f"epoch:{epoch+1}, Gen_loss: {np.mean(Gen_loss)}")
...
def val(args, generator, registor, dataloader, device):
    ...
    for i, (ct_data, cta_data) in loop:
            ...
            split_ct_img = ct_img.split(4, dim=0) 
            for ct in split_ct_img:
                ct =ct.repeat(1,3,1,1).to(device)
                with torch.no_grad():
                    # flow = registor(ct)
                    # reg_ct = transformer(ct, flow)
                    lattenx = backbone(ct)
                    fake_cta = generator(lattenx)
