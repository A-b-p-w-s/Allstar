...
from model.attention_unet import AttResUNet
...
def get_args():
    ...
    parser.add_argument('--batch_size', type=int, default=8, help='batch size, here must should be 1')
...
def train(args, generator, registor, discriminator, train_dataloader, val_dataloader,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, data_dict in loop:
            ct_img = data_dict['ct_imgs'].to(device)
            cta_img = data_dict['cta_imgs'].to(device)
            ...
            for n in range(1):
                ...
                with torch.no_grad():
                    ...
                    fake_cta = generator(ct_img).detach()
                real_logits = discriminator(cta_img)
                fake_logits = discriminator(fake_cta)
                gradient_penalty_loss = gradient_penalty(discriminator, cta_img, fake_cta, device=device)
            ...
            fake_cta = generator(ct_img) 
            fake_logits = discriminator(fake_cta)
            g_loss = generator_loss(fake_logits)
            Gen_loss.append(g_loss.item())
            mae_loss = F.l1_loss(fake_cta, cta_img)
            ...
            PSNR.append(psnr(fake_cta,cta_img).item())
...
def val(args, generator, registor, dataloader, device):
    ...
    for i, data_dict in loop:
            ct_img = data_dict['ct_imgs'].to(device)
            cta_img = data_dict['cta_imgs'] 
            with torch.no_grad():
                fake_cta = generator(ct_img).detach()

            fake_cta = fake_cta.cpu()
            ssim = ssim_metric(cta_img, fake_cta)
...
if __name__ == '__main__':
    ...
    train_dataset = DICOM_Dataset(ct_dir=args.val_ct_dir, cta_dir=args.val_cta_dir,
