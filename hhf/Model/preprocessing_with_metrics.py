...
file_list = []
for root, _, filenames in os.walk('new_livermodel/Task03_Liver/imagesTr'):
...
for i in range(len(df_files)):  # Assuming you have files for each CT and mask
    ct = f"liver_{i}.nii.gz"
    mask = f"liver_{i}.nii.gz"
    df_files.loc[df_files['filename'] == ct, 'mask_filename'] = mask
    df_files.loc[df_files['filename'] == ct, 'mask_dirname'] = "new_livermodel/Task03_Liver/labelsTr"
...
# Iterate through each CT and corresponding mask
for idx, row in df_files.iterrows():
    ct_file = row['dirname'] + "/" + row['filename']
    mask_file = row['mask_dirname'] + "/" + row['mask_filename']
    ...
    for slice_idx in range(ct_data.shape[2]):
        ...
        if np.any(ct_slice != 0):
            ...
            ct_tensor.save_tiff_512x512(f"train_images/{row['filename'].split('.')[0]}_slice_{slice_idx}.tiff", 
                                        [dicom_windows.liver, dicom_windows.custom])

            # Save mask slice as grayscale TIFF
            mask_tensor = tensor(mask_slice.astype(np.float32))
            mask_tensor.save_tiffgray_512x512(f"train_masks/{row['filename'].split('.')[0]}_slice_{slice_idx}.tiff", 
                                              [dicom_windows.liver, dicom_windows.custom])
...
# Optionally, you can save the metrics to a file or use them further as needed.

# Example of saving metrics to a CSV file
metrics_dict = {
...
# Optionally, perform additional processing or analysis here based on your requirements.
# For example, you may want to visualize some slices, perform statistical analysis, etc.
